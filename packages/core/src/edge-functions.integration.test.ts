...
function readIntegrationEnv() {
  ...
  return {
    ...
    serviceRoleKey,
    edgeBaseUrl:
      process.env.AURA_EDGE_BASE_URL ?? `${supabaseUrl.replace(/\/$/, '')}/functions/v1`,
...
async function createTenant(label: string) {
  ...
  const email = `edge-integration+${label}-${randomUUID()}@aura.test`;
...
async function invokeWebhook(edgeBaseUrl: string, payload: Record<string, unknown>) {
  ...
  cases: [
    {
      ...
      run: async () => {
        ...
        assert.ok(conversationResponse.data);
        assert.equal(messagesResponse.data?.length ?? 0, 1);
      },
    },
    {
      ...
      run: async () => {
        ...
        const messageResponse = await tenant.service
          ...
          .insert({
            ...
            external_message_id: `wamid.status.${randomUUID()}`,
        ...
        assert.equal(updatedMessageResponse.data?.status, 'delivered');
        assert.equal(eventsResponse.data?.length ?? 0, 1);
      },
    },
    {
      ...
      run: async () => {
        ...
        assert.equal((auditResponse.data?.length ?? 0) > 0, true);
      },
    },
    {
      ...
      run: async () => {
        ...
        assert.equal(
          typeof (sendResponse.data as Record<string, unknown> | null)?.message === 'object',
          true,
        );
      },
    },
